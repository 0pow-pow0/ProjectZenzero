...
/// </summary>
public class CardHealthPowerUp : Card, ICardUpgradableOneTime<int>
{
    #region SINGLETON SHIT
    private static CardHealthPowerUp instance;
    public static CardHealthPowerUp GI()
    {
        if (instance == null)
        {
            //Debug.LogError("CardHealthPowerUp non inizializzato!");
            instance = new CardHealthPowerUp();
        }
    ...
    private CardHealthPowerUp() :
        base(CardSignature.CardHealthPowerUp)
    {
    }
    ...
    {
        base.Setup("CardHealthPowerUpFather");
        ...
        lvl1Sprite.SetSprites(parent, GameManager.GI().GetSprite("CardFrontPowerUpStat"),
            GameManager.GI().GetSprite("CardArtworkHeartRed"),
            GameManager.GI().GetSprite("CardArtworkHeartRed"),
            GameManager.GI().GetSprite("CardArtworkHeartRed"));
    ...
    public override void UpgradeLevelAction()
    {
        ...
        GameManager.GI().plrScr.maxHP -= cardLevelUpValues[level];
        level++;
        GameManager.GI().plrScr.maxHP += cardLevelUpValues[level]; 
