...
/// </summary>
public class CardSpeedPowerUp : Card, ICardUpgradableOneTime<float>
{
    #region SINGLETON SHIT
    private static CardSpeedPowerUp instance;
    public static CardSpeedPowerUp GI()
    {
        if (instance == null)
        {
            //Debug.LogError("CardHealthPowerUp non inizializzato!");
            instance = new CardSpeedPowerUp();
        }
    ...
    private CardSpeedPowerUp() :
        base(CardSignature.CardSpeedPowerUp)
    {
    }
    ...
    {
        base.Setup("CardSpeedPowerUpFather");
        ...
        lvl1Sprite.SetSprites(parent, GameManager.GI().GetSprite("CardFrontPowerUpStat"),
            GameManager.GI().GetSprite("CardArtworkSpeed"),
            GameManager.GI().GetSprite("CardArtworkSpeed"),
            GameManager.GI().GetSprite("CardArtworkSpeed"));
    ...
    public override void UpgradeLevelAction()
    {
        ...
        GameManager.GI().plrScr.speed -= cardLevelUpValues[level];
        level++;
        GameManager.GI().plrScr.speed += cardLevelUpValues[level];
